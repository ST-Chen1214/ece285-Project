...
from diffusers.models import AutoencoderKL, ImageProjection, UNet2DConditionModel, UNet3DConditionModel
...
from diffusers.models import AutoencoderKL, ImageProjection, UNet2DConditionModel, UNet2DModel, VQModel
...
class StableDiffusionXLPipeline(
    ...
    def __init__(
        ...
        # CAS-specific state
        self.cas_prev_noise_pred = None
        self.cas_current_delta_t = 1
        self.cas_gamma_dec = 0.8
        self.cas_gamma_inc = 1.25
        self.cas_min_delta = 1
        self.cas_max_delta = max_skip_steps
        self.cas_skip_counter = 0
    ...
    def estimate_skipping(self, latent):
        """CAS-compatible stub; actual skip logic is in the denoising loop."""
        if self.cas_skip_counter > 0:
            self.cas_skip_counter -= 1
            return False  # skip (reuse cached noise pred)
        return True   # run UNet

    def cas_update_delta(self, kappa):
        """Feedback controller: adjust delta_t based on curvature."""
        if kappa > self.threshold:
            new_delta = self.cas_current_delta_t * self.cas_gamma_dec
        else:
            new_delta = self.cas_current_delta_t * self.cas_gamma_inc
        self.cas_current_delta_t = int(round(
            max(self.cas_min_delta, min(self.cas_max_delta, new_delta))
        ))
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ###### CAS: Curvature-Adaptive Sampling ######
                # Warm-up: always run UNet for first 2 steps
                should_run_unet = (len(self.prev_latents) < 2) or (self.cas_skip_counter <= 0)
                ...
                if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                    ...
                    noise_pred_cfg = rescale_noise_cfg(noise_pred_cfg, noise_pred_text, guidance_rescale=self.guidance_rescale)

                # CAS curvature estimation and delta update (only when UNet was run)
                if should_run_unet and self.cas_prev_noise_pred is not None:
                    kappa = (noise_pred_cfg - self.cas_prev_noise_pred).norm().item()
                    self.cas_update_delta(kappa)
                    if self.cas_current_delta_t > 1:
                        self.cas_skip_counter = self.cas_current_delta_t - 1
                if should_run_unet:
                    self.cas_prev_noise_pred = noise_pred_cfg.detach().clone()

                # compute the previous noisy sample x_t -> x_t-1
                latents_dtype = latents.dtype
                latents = self.scheduler.step(noise_pred_cfg, t, latents, **extra_step_kwargs, return_dict=False)[0]
                ...
                ##################################
                # Record mask (True = UNet ran) for compatibility with generate.py
                self.mask.append(should_run_unet)
